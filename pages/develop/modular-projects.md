...
First and foremost is better support for all SiFive device peripherals; new definitions of the memory mapped registers are already under way; APIs for new device drivers are currently in design phase, and will be implemented soon (as soon as the OpenCOD issues the hamper development will be addressed).
...
In addition to the simple blinky projects generated now, the future templates should be able to generate more complex projects, including a RTOS and possibly some middleware (like POSIX IO, File System, Network).

Support for FreeRTOS and µOS++ is planned, with the focus being on µOS++, as it supports both C and C++.
...
The SiFive C/C++ Project template is only the first step of using xPacks in Eclipse. 

After the command line tools will be fully functional, more xPack support will be added to Eclipse, first to complement the CMSIS Packs, then possibly as an alternate solution to the managed build support available now in Eclipse CDT.

### Integration with other editors/IDEs

The main xPack tools will continue to be developed as command line tools.

However, new editors and graphical IDEs are under continuous developpment and some of them get more and more traction (for example Visual Studio Code), so using them to build applications based on xPacks may become an attractive option.
...
The first step is to emulate the board at such a level that not only allows the 'blinky' projects generated by the template to run, but also to show the LEDs blinking on top of the board image, and to trigger button actions when the button image is clicked on.

In order to do this, the structured peripheral registers definitions available in the device xPack need to be validated, since the actual QEMU implementation code is automatically generated from the xsvd definitions.

### Documentation pages

The current code already includes lots of Doxygen definitions, aiming to allowing the full reference manuals to be generated from the source code, using the same approach as for the existing [µOS++ reference pages](http://micro-os-plus.github.io/reference/cmsis-plus/).
